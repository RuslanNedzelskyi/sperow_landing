...
import '../App.css';
import { useTranslation } from 'react-i18next';
import agroAiImage from '../assets/images/agro_ai.jpg';

interface IAgroAiProps {
  handleOpenModal: () => void;
}

const AgroAi = forwardRef<HTMLDivElement, IAgroAiProps>(({ handleOpenModal }, ref: Ref<HTMLDivElement>) => {
  const { t } = useTranslation();

  return (
    <div className="agro_ai_page" ref={ref}>
      <div className="agro_ai_content">
        <div className="agro_ai_text">
          <h2 className="section-title">{t('Agroassistant')}</h2>
          <p className="agro_ai_description">{t('AgroAI_Description')}</p>
          <button className="cta-button" onClick={handleOpenModal}>
            {t('Test_MVP')}
          </button>
        </div>
        <div className="agro_ai_image_container">
          <img src={agroAiImage} alt="Agro AI" className="agro_ai_image" />
        </div>
      </div>
    </div>
  );
