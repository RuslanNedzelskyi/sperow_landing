...
const Values = forwardRef<HTMLDivElement>((props, ref) => {
  ...
  const imageSrc = i18n.language === 'jp' ? valuesJp : i18n.language === 'en' ? valuesEn : values;

  return (
    <div ref={ref} className="values_page">
      <div className='values_image_container'>
        <img className="values_image" src={imageSrc} alt="Values" />
      </div>
    </div>
  );
});
