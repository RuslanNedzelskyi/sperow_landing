...
import { useTranslation } from "react-i18next";
import { CssBaseline, Backdrop } from "@mui/material";
import MenuIcon from "@mui/icons-material/Menu";
import CloseIcon from "@mui/icons-material/Close";
      ...
      <div className="App">
        <header className="header">
          <div className="header-content">
            <img src={logo} alt="Sperow Logo" className="logo" />
          ...
          {menuOpen && (
            <nav className="mobile-nav">
              <ul>
                <li onClick={() => scrollTo(generalRef)}>{t("General")}</li>
                <li onClick={() => scrollTo(aboutRef)}>{t("About_Us")}</li>
                <li onClick={() => scrollTo(agroRef)}>{t("Agroassistant")}</li>
                <li onClick={() => scrollTo(techRef)}>{t("Process_Maps")}</li>
                <li onClick={() => scrollTo(ecosystemRef)}>{t("Ecosystem")}</li>
                <li onClick={() => scrollTo(missionRef)}>{t("Missions_Goals")}</li>
                <li onClick={() => scrollTo(advantagesRef)}>{t("Advantages")}</li>
                <li onClick={() => scrollTo(valuesRef)}>{t("Our_Values")}</li>
                <li onClick={() => scrollTo(partnersRef)}>{t("Partners")}</li>
                <li onClick={() => scrollTo(teamRef)}>{t("Team")}</li>
              </ul>
            </nav>
          )}
        </header>
        ...
        <main>
          <General ref={generalRef} handleOpenModal={() => setModalOpen(true)} />
          <AboutUs ref={aboutRef} handleOpenModal={() => setModalOpen(true)} />
          <AgroAi ref={agroRef} handleOpenModal={() => setModalOpen(true)} />
          <TechCard ref={techRef} />
          <Ecosystem ref={ecosystemRef} />
          <Mission ref={missionRef} handleOpenModal={() => setModalOpen(true)} />
        ...
        <Footer scrollToSection={scrollToById} handleOpenModal={() => setModalOpen(true)} />

        <ModalMvp open={modalOpen} handleClose={() => setModalOpen(false)} />
